...
class DeltaRestClient:
  ...
  def create_bracket_order(self, product_id=None, product_symbol=None, stop_loss_order=None, take_profit_order=None, bracket_stop_trigger_method="last_traded_price"):
    """
    Create bracket orders (stop loss and take profit) for an existing position
    
    Args:
      product_id: Product ID (optional if product_symbol provided)
      product_symbol: Product symbol (optional if product_id provided)
      stop_loss_order: Dict with order_type, stop_price, limit_price (optional)
      take_profit_order: Dict with order_type, stop_price, limit_price (optional)
      bracket_stop_trigger_method: Trigger method (default: "last_traded_price")
    ...
    if product_id:
      bracket_payload["product_id"] = product_id
    elif product_symbol:
      bracket_payload["product_symbol"] = product_symbol
    else:
      raise Exception("Either product_id or product_symbol must be provided")
    
    if stop_loss_order:
      bracket_payload["stop_loss_order"] = stop_loss_order
    
    if take_profit_order:
      bracket_payload["take_profit_order"] = take_profit_order
    
    response = self.request('POST', "/v2/orders/bracket", bracket_payload, auth=True)
    return parseResponse(response)
