...
stop_with_pid_file() {
    ...
    if [ -f "$pid_file" ]; then
        ...
        if pkill -f "$fallback_pattern" >/dev/null 2>&1; then
...
stop_with_pid_file "backend" "$PROJECT_ROOT/logs/backend.pid" "uvicorn.*main:app"
stop_with_pid_file "frontend" "$PROJECT_ROOT/logs/frontend.pid" "next start"
